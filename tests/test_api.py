...
def test_scan_confirm_and_learning_status(tmp_path: Path) -> None:
    ...
    with TestClient(app) as client:
        ...
        assert status["active_model"] == "chess-steps-v4"
...
def test_startup_rejects_corrupted_active_model(tmp_path: Path) -> None:
    ...
    model_dir.mkdir()
    base_path = model_dir / "chess-steps-v4.onnx"
    base_metadata_path = model_dir / "chess-steps-v4.json"
    shutil.copyfile(PROJECT_ROOT / "models/chess-steps-v4.onnx", base_path)
    shutil.copyfile(PROJECT_ROOT / "models/chess-steps-v4.json", base_metadata_path)
    settings = Settings(
        data_dir=tmp_path / "data",
        model_dir=model_dir,
        web_dist=tmp_path / "web",
