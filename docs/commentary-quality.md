...
The optional `commentary-planner-1` integration is a bounded selector, not a chess analyst or prose generator. The deterministic review is returned first. A separate coaching request sends an OpenAI-compatible endpoint only the FEN, checked Stockfish lines, deterministic evidence atoms, and pre-rendered claim candidates. It never sends a photograph, scan or feedback ID, user text, article annotation, or training record.

The model may return only one or two allowed claim IDs plus a fixed focus enum. Unknown IDs, extra fields, malformed JSON, invalid provider envelopes, bounded end-to-end timeouts, oversized responses, and provider failures produce a deterministic fallback. Unexpected internal planner defects are logged and are not cached as final runs. Scores, verdicts, moves, diagrams, and claim wording cannot be changed by the model. One immutable run is retained per review with provider/model/prompt versions, the exact sanitized provider request, raw output, accepted IDs, latency, token usage, and a sanitized failure code. The API key is never persisted.
