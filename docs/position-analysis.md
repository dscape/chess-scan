...
8. The deterministic review returns immediately. If an operator explicitly enables deeper coaching, the frontend separately asks a bounded model to select one or two already verified claims. The server composes those facts into an evidence-cited diagnosis, continuation, better option, and practical calculation cue; every interactive SAN token replays one checked line ply. The model cannot write claims, alter verdicts, or create diagrams.
...
After the first-move review, study mode returns to the scanned position and lets the learner play legal moves for either side. Each settled position receives a stable two-candidate local MultiPV search. The first choice stays visible as a separate engine overlay, while the API compiles any root-ply tactical geometry from the checked line without storing another review event. Position analysis is cached by canonical FEN, while path-specific move grades are cached separately so transpositions reuse Stockfish work without confusing repetition-dependent grades. Rank-one moves are graded from the cached parent review unless a history-dependent draw requires a fresh comparison; other moves use a lightweight ephemeral attempt comparison containing one best line. A retry preserves successful engine and API results and repeats only the failed request, while permanent request defects remain explicit and are not retried.

Move grades remain selective. A best move is brilliant only when the evidence detector ties it to a sound temporary material sacrifice. It receives an only-move mark when the second engine candidate loses more than five percentage points of expected score. Inaccuracies and mistakes share the bad-move mark; blunders retain the review pipeline's blunder verdict. Acceptable moves that meet none of those conditions receive no badge.
